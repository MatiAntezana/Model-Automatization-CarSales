...
def new_path_results(type_model_user, data_user, features_user):
    ...
    pattern = re.compile(r"model_(\d+)_path")
    existing_numbers = []
    ...
    new_folder_name = f"model_{new_number}_path"
    ...
    file_txt_path = os.path.join(new_folder_path, f"param_used_{new_number}.txt")
    
    with open(file_txt_path, 'w') as file_txt:
